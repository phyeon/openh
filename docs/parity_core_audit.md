...
| `[~]` | `openh/memdir.py` | `crates/core/src/memdir.rs` | Reviewed this pass. Recursive memory scanning, quick frontmatter parsing, MEMORY.md truncation, index-only prompt injection, and public-style auto-memory gating (`CLAURST_DISABLE_AUTO_MEMORY`, `CLAURST_SIMPLE`, remote memory env checks) now track the public memdir flow more closely. Still does not expose the full public relevance-search helper surface. |
...
| Status | OpenH file | Primary public reference | Notes |
| --- | --- | --- | --- |
| `[~]` | `openh/permission_rules.py` | `crates/core/src/lib.rs` | Reviewed again line-by-line. Manager-backed default behavior now matches public flow more closely: explicit deny/allow first, then mode fallback (`read -> allow`, `write/exec/network -> ask or deny`). Non-interactive sessions now force the auto handler even if they inherit `interactive`, WebFetch/WebSearch rules now actually match URL/query patterns, coordinator-banned tools are denied at runtime for the manager session, and persistent rules now load/save the public-style `permission_rules` object list while still reading legacy local `permissions.allow/ask/deny` buckets. Still not a literal Rust port. |
